...
MappingFsm = machina.Fsm.extend({
    ...
    // initialize the map
    initialize: function(canvasElementId, readyCallback) {
    ...
    _resizeMarkerById: function (id, size) {
      var marker = null;
...
debugger
      if (marker) {
        ...
        // Remove large class from al markers => default size
        mapDiv.find(".map-marker").removeClass("large").css("z-index", 1);

        // add the new size to the selected marker => "large" is 
        // currently the only acceptable option
        if (! _.isUndefined(size)) {
          $(marker.$div).addClass(size).css("z-index", 2);
        } 
      }
    },  
    ...
    states: {
      ...
      showActivity: {
        ...
        "map.medium": function() {
          ...
          this._setContainerClass('medium', function () {
            // Set marker to large size
            self.handle("marker.large");
          });
        },
        "marker.large": function() {
          this._resizeMarkerById(this.activityId, "large");
      ...
      showActivityIdle: {
        ...
        _onExit: function() {
          debugger
          this._resizeMarkerById(this.activityId);
        },
      }
    }
});